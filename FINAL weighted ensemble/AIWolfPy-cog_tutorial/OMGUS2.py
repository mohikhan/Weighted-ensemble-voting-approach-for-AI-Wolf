...
from weighted_ensemble_voting import predict_vote

#Saving the training data in log file named "training data.log"
myname = 'Testing agent'.format(randint(0,99)) 
...
class SampleAgent(object):
    ...
    def initialize(self, base_info, diff_data, game_setting):
        ...
        self.mytarget=5  #targeting agent 1
    ...
    def update(self, base_info, diff_data, request):
        ...
        for row in diff_data.itertuples():
            ...
            if (type == "vote"):
                ...
                #         if target == self.myid:
                #             logging.debug("{}  , {}  , {}  , {}  ".format(self.n_talks[4],self.p_talks[4],self.n_len[4],1))
                ...
                #             logging.debug("{}  , {}  , {}  , {}  ".format(self.n_talks[4],self.p_talks[4],self.n_len[4],0))
                #             # logging.debug("The FINAL value of counter +ve is {}".format(self.counter_positive[0]))
        ...
        if (request == "DAILY_INITIALIZE"):
            logging.debug("Day is initialized") 
            ...
            self.n_talks = [0]*5
            self.n_len = [0]*5
            self.result = [0]*self.no_of_players
        ...
        for i in range(self.no_of_players):
            extract_update(base_info,diff_data,request,self.myid,self.n_talks,self.p_talks,self.n_len,i+1)
        ...
        for p in range(self.no_of_players):
            ...
            # self.result[p] = predict_vote(9,0,145)
            # logging.debug("The result is {}".fomat(0))
            logging.debug("{}   {}    {}  {}".format(self.n_talks[p],self.p_talks[p],self.n_len[p],self.result[p]))

        for k in range(self.no_of_players):
            if(self.result[k] == 1):
                # flag = 1
                self.hate = k + 1
                logging.debug("Hate called")
        
        

        logging.debug("*****************************************************")

        # logging.debug("Result is: ",self.ntalks[0])
